...
Validation-V3: get XML files and validate this one according to a XSD and a Schematron files.

3) Howto use
------------
Multiple Run DevOps:
- Unzip the file locally (ex: c:/Calimero20). After unzipping, rename the file curl._xe to curl.exe (was renamed to avoid mail problems and this tool is needed to update installation).
- Copy all the .edi and .txt you want to validate in the subfolder 'files'. ONLY VERSION 3 HO2C CAN BE VALIDATED. Version 2 and earlier will be reported as error.
- Dubble click on RunMultipleDevOps.bat (most recent jar-, xsd-, sch- and codelistfiles will be updated first from GS1 Belgium & Luxembourg DevOps by using curl).
...
- For every .edi and .txt file, 2 files are created (or overwritten). An .err file holding the same messages as reported on the screen and an .xml file holding the transformed .edi/.txt file.
